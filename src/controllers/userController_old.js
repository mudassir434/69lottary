...
const listMyTeam = async (req, res) => {
    ...
    let userInfo = user[0];
    const [f1] = await connection.query('SELECT `id_user`, `name_user`,`status`, `roses_f`,`time` FROM users WHERE `invite` = ? ORDER BY id DESC', [userInfo.code]);
...
const withdrawal3 = async (req, res) => {
    ...
    let money = req.body.money;
    let password = req.body.password;
    if (!auth || !money || !password || money < 10) {
        return res.status(200).json({
            message: 'Failed',
            status: false,
            timeStamp: timeNow,
